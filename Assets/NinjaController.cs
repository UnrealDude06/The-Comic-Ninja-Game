...
public class NinjaController : MonoBehaviour
{
    ...
    public bool itemSeen;
    public bool itemHit;
    public float HomingAttackRadis;
    public LayerMask InteractLayer;
    public GameObject homingAttackPos; 
    ...
    [Header("Stomp")]
    public bool stomp;
    public float stompDownSpeed;        


    ///ninja related attacks
    [Header("Fist Attack/ Left Click")]
    float attackTimer; 
    float timeStamp;
    ...
    [Header("adjust player to enemy when attacking")]
    public GameObject nearEnemyCheck;
    public float nearEnemyCheckRadius,lerpSpeed;
    
    [Header("Item Attack/ Right Click")] 
    public bool attack_secondary;
    public GameObject ninjaStar,throwPos;
...
void Dash()
{
    if(Input.GetButtonDown("Shift") && !dash && !isGrounded)
    {
...
void DoHomingAttack()
{
 ...
 Collider[] hitColl = Physics.OverlapSphere(transform.position, HomingAttackRadis,InteractLayer);
        if (hitColl.Length != 0)
        {

        Debug.Log("Found item to homing attack on!");
        itemSeen = true;
        }
         if (hitColl.Length < 1)
        {
        ...
        if(hitColl.Length !=0)
        {
            foreach(Collider nearObj in hitColl)
                {
                    
                    if(Input.GetButtonDown("Shift") && itemSeen)
                    {
                        rb.MovePosition(nearObj.gameObject.transform.position);
                        itemHit = true;
                        canControl = true;
...
void MeleeAttack()
     {
         ...
         if(Input.GetButtonDown("Jump") && combo >0)
         {
            canControl = true;
         }
         ...
         ////////////find enemy if plyaer attacks them and move player to enemy
           ...
           Collider[] colliderMove = Physics.OverlapSphere(nearEnemyCheck.transform.position, nearEnemyCheckRadius,InteractLayer);
            if (colliderMove.Length != 0)
            {
                ...
                foreach(Collider nearEnemy in colliderMove)
                    ...
                    if(Input.GetButtonDown("Fire1") )
                        ...
                        rb.MovePosition(Vector3.Lerp(transform.position,nearEnemy.gameObject.transform.position,lerpSpeed * Time.deltaTime));
    ...
    void RMB_Attack()
    {
        ...
        if(Input.GetButtonDown("Fire2") && !attack_secondary)
        {
            attack_secondary =   true;
        }

        if(attack_secondary == true)
        {
            ///throw noinja stars
             Instantiate(ninjaStar, throwPos.transform.position, transform.rotation);
         ...
         private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(homingAttackPos.transform.position,HomingAttackRadis);
        Gizmos.color = Color.blue;
        Gizmos.DrawWireSphere(nearEnemyCheck.transform.position,nearEnemyCheckRadius);
        Debug.DrawRay(transform.position,transform.right * 1f,Color.magenta);
    }
